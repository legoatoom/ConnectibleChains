...
public class ChainTypes implements SidedResourceReloadListener<List<ChainType>> {
    ...
    public ChainType create(Identifier id, Identifier texture, Identifier knotTexture) {
        return create(id, texture, knotTexture, UVRect.DEFAULT_SIDE_A, UVRect.DEFAULT_SIDE_B);
    }

    @Environment(EnvType.CLIENT)
    public ChainType create(Identifier item, Identifier texture, Identifier knotTexture, UVRect uvSideA, UVRect uvSideB) {
