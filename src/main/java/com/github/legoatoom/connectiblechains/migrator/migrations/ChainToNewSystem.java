...
public final class ChainToNewSystem {
    ...
    public static <E extends BlockAttachedEntity & Chainable> void migrate(ReadView readView, WriteView writeView, E entity) {
        Item knotSourceItem = readView.getOptionalString(OLD_SOURCE_ITEM_KEY).map(sourceKey -> Registries.ITEM.get(Identifier.tryParse(sourceKey))).orElse(Items.CHAIN);
    ...
    private static <E extends BlockAttachedEntity & Chainable> void migrateChainData(ReadView readView, WriteView writeView, E entity) {
        Item source = readView.getOptionalString(OLD_SOURCE_ITEM_KEY).map(sourceKey -> Registries.ITEM.get(Identifier.tryParse(sourceKey))).orElse(Items.CHAIN);
